Shader "Unlit/WorldSwitch"
{
    Properties
    {
        ...
        [HideInInspector] _FirstWorldCulling ("First world culling", 2D) = "black" {}
        [HideInInspector] _SecondWorldCulling ("Second world culling", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _FirstWorldCulling;
            sampler2D _SecondWorldCulling;
            
            float4x4 _InverseRotationMatrix;
            
            fixed _WorldSwitchRadius;
            ...
            {                
                ...
                fixed4 firstWorldCulling = tex2D(_FirstWorldCulling, i.uv);
                fixed4 secondWorldCulling = tex2D(_SecondWorldCulling, i.uv);

                fixed firstIsFirst = length(worldSpacePoint1.xz) >= _WorldSwitchRadius;
                fixed secondIsSecond = length(worldSpacePoint2.xz) < _WorldSwitchRadius;
                ...
                /*col = abs(length(worldSpacePoint1.xz) - _WorldSwitchRadius) < 0.05
                    || abs(length(worldSpacePoint2.xz) - _WorldSwitchRadius) < 0.05
                        ? fixed4(0, 1, 0, 1)
                        : col;*/
                
                //ol = firstIsClosest && firstIsFirst;
